...
def parser():
    ...
    """
    parser = argparse.ArgumentParser(description='Sort and merge ligpargen xml files for OpenMM input.')

    # Define required arguments
    parser.add_argument('-x','--xml_files', nargs='+', help='List of input XML files to be processed and merged.')
    ...
    for xml in xml_files:
        ...
        for element in tree.findall('AtomTypes'):
            for subelement in element:
                ...
                os.system('sed -i "s/%s/%s/g" %s' % (type_name, new_type_name, sorted_xml_file))
                class_name = subelement.attrib.get('class')
                if resname not in class_name:
                    os.system('sed -i "s/%s/%s_%s/g" %s' % (class_name, class_name, resname, sorted_xml_file))
...
def merge_xmls(sorted_xml_files, output_dir):
    ...
    base_names = [os.path.splitext(os.path.basename(file.replace("_sorted","")))[0] for file in sorted_xml_files]
